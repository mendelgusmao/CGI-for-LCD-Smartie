Attribute VB_Name = "mdlSocketMaster"
'**************************************************************************************
'
'mdlSocketMaster module 1.1
'Copyright (c) 2004 by Emiliano Scavuzzo <anshoku@yahoo.com>
'
'Rosario, Argentina
'
'**************************************************************************************
'This module contains API declarations and helper functions for the SocketMaster class
'**************************************************************************************
...
If Not m_blnInitiated Then
    ...
    Else
        ...
        Err.Raise lngResult, "mdlSocketMaster.InitiateProcesses", GetErrorDescription(lngResult)
...
If lngResult = 0 Then
    ...
    Err.Raise sckOutOfMemory, "mdlSocketMaster.DestroyWinsockMessageWindow", "Out of memory"
Else
...
If m_colSocketsInst Is Nothing Then
    ...
    If CreateWinsockMessageWindow <> 0 Then
        Err.Raise sckOutOfMemory, "mdlSocketMaster.RegisterSocket", "Out of memory"
    End If
...
If blnEvents Then
    ...
    If lngResult = SOCKET_ERROR Then
        ...
        lngErrorCode = Err.LastDllError
        Err.Raise lngErrorCode, "mdlSocketMaster.RegisterSocket", GetErrorDescription(lngErrorCode)
    Else
...
'It turns a SocketMaster instance pointer into an actual instance.
Private Function SocketObjectFromPointer(ByVal lngPointer As Long) As SocketMaster

Dim objSocket As SocketMaster
...
'Assing a temporal instance of SocketMaster to a
...
Public Function GetAcceptClass(ByVal lngSocket As Long) As SocketMaster
